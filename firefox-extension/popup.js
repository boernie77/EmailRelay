...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    // Tab-URL nicht verfügbar (z.B. about:blank) — ignorieren
  }
  ...
  try {
    ...
    if (resp.status === 403) {
      throw new Error("API-Secret falsch — Einstellungen prüfen.");
    }
    if (!resp.ok) {
    ...
    if (!addresses.length) {
      showError(
        "Keine Adressen konfiguriert. Bitte in E-Mail Relay eine Adresse anlegen."
      );
      return;
    }
  ...
  document.getElementById("copy").addEventListener("click", async () => {
    ...
    setTimeout(() => {
      document.getElementById("copy").textContent = "Nochmal kopieren";
    }, 1500);
  });
});
...
async function createAlias() {
  ...
  try {
    ...
    try {
      ...
      // Kein Content-Script auf dieser Seite — ignorieren
    }
